...
5. Declare the contract, helpers, configs, and local transitive dependencies in the policy's `controlAssets`; then freeze them outside the candidate workspace and point the reusable Delivery Gate workflow at `contracts/api-v1.json` by immutable control commit.
...
`tcpListenerBindings` uses host `lsof` output scoped to the child PID and port to distinguish an exact loopback bind from a wildcard listener. Contracts that call it must make `lsof` a runner prerequisite. The helper, contract, and policy can be bundled; the `lsof` binary and kernel cannot, so execution-environment provenance remains separate evidence.
